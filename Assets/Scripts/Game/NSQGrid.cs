...
namespace NSQ
{
    public partial class NSQGrid : MonoBehaviour
    {
        ...
        private void NSQCheckAdjacentBalls(Vector2Int gridPosition, ENSQColor color,
            ref HashSet<NSQTile> ballsToDestroy)
        {
            if (gridPosition.x < 0 || gridPosition.x >= NSQSize.x || gridPosition.y < 0 ||
                gridPosition.y >= NSQSize.y || _nsqActiveTiles[gridPosition] == null ||
                _nsqActiveTiles[gridPosition].NSQColor != color ||
                ballsToDestroy.Contains(_nsqActiveTiles[gridPosition]))
                return;

            ballsToDestroy.Add(_nsqActiveTiles[gridPosition]);

            var x = gridPosition.x;
            var y = gridPosition.y;

            NSQCheckAdjacentBalls(new Vector2Int(x + 1, y), color, ref ballsToDestroy);
            NSQCheckAdjacentBalls(new Vector2Int(x - 1, y), color, ref ballsToDestroy);
            NSQCheckAdjacentBalls(new Vector2Int(x, y + 1), color, ref ballsToDestroy);
            NSQCheckAdjacentBalls(new Vector2Int(x, y - 1), color, ref ballsToDestroy);
        }
